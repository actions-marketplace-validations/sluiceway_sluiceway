...
A queued row starts with `spinner-queued-light.svg` or `spinner-queued-dark.svg` instead: the same crate, level and still, in still water (record 0098). The queued header ties the ticked crate up at the closed gate and nothing moves but the water, so the row's crate does not move either, and motion on a row always means that stack is deploying now.
