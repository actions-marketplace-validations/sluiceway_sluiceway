...
// The file rules of record 0033, checked in CI on every header image. The
// cap forces clean, hand-made SVG and keeps the header instant on a phone.

const DIR = resolve(import.meta.dir, "../../assets/mascot");
const MAX_BYTES = 10 * 1024;
const FILES = HEADER_STATES.flatMap((state) => [`${state}-light.svg`, `${state}-dark.svg`]);

function violations(name: string, svg: string): string[] {
  const found: string[] = [];
  const bytes = new TextEncoder().encode(svg).length;
  if (bytes > MAX_BYTES) found.push(`${bytes} bytes, over 10 KB`);
  if (!/^<svg[^>]*\sviewBox="0 0 440 120"/.test(svg)) found.push("view box is not 0 0 440 120");
...
describe("the check itself", () => {
  const svg = (inside: string) =>
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 440 120">${inside}</svg>`;
  ...
  test("flags another view box", () => {
    expect(violations("a.svg", '<svg viewBox="0 0 880 240"></svg>')).toEqual([
      "view box is not 0 0 440 120",
    ]);
  });
...
test("there is one light and one dark file for every header state, and no other image", () => {
