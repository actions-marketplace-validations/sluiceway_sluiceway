...
//
//   bun run record:fixtures [--out <dir>] [--work-dir <dir>]
//                           [--expect-version v3.229.0] [--only <scenario>]
//
...
const { values } = parseArgs({
  options: {
    out: { type: "string", default: join(REPO, "test/fixtures/pulumi") },
...
const cliVersion = found.stdout.trim();
if (found.exitCode !== 0 || !/^v\d+\.\d+\.\d+$/.test(cliVersion)) {
  throw new Error(`"pulumi version" did not give a release version. Is the CLI on PATH?`);
}
if (values["expect-version"] !== undefined && values["expect-version"] !== cliVersion) {
  throw new Error(`Expected pulumi ${values["expect-version"]} on PATH, found ${cliVersion}.`);
}
...
const outDir = join(resolve(values.out), cliVersion);
...
for (const scenario of scenarios) {
  ...
  await recordScenario(scenario, {
    exampleDir: join(REPO, "examples/pulumi-basic"),
    workDir,
    outDir,
    cliVersion,
  ...
  const found = checkRecording(join(outDir, scenario.name), scenario);
  problems.push(...found);
...
if (problems.length > 0) {
  console.error(
    `\nThe recording with pulumi ${cliVersion} does not show what the scenarios are for:`,
  );
  for (const problem of problems) console.error(`- ${problem}`);
  process.exit(1);
}
console.log(`\nRecorded ${scenarios.length} scenarios with pulumi ${cliVersion} into ${outDir}`);
